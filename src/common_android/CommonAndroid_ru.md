...
4. **Android Runtime (среда выполнения)** - Каждое приложение работает в своем собственном
   ...
   ART ([Dalvik vs ART](#2---dalvik--art)).
...
Чтобы решить эти недостатки, в Android Nougat JIT (Just In Time) была возвращена, и объеденена с AOT, что сделало его **гибридным**.
Используя гибридную среду выполнения (Hybrid Runtime), во время установки не будет выполняться компиляция.
